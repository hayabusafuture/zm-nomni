...
- A missing-UOM suggestion makes its extra step clear in the action area: a primary action such as `Add carton`, followed by `Enter how many kilograms are in one carton`. Its dialog explains that the invoice uses that unit and asks how much one unit contains before the line is added. Both suggestion types use the same completed treatment after resolution. The older Admin-only behaviour that exposes every possible UOM in the normal line dropdown is not included.
