...
| Outlet groups | Group row actions | Partial | Three-dot menus expose Add outlets, Settings and Delete group; Add outlets opens a search-first in-page picker that includes ungrouped outlets already managed by this HQ as well as eligible Procure outlets, with company and cluster filters, capped results and select/deselect-all | Replace the prototype catalogue with server-side search and pagination |
...
| Outlet assignment | Add outlets from group editor | Partial | Search, filters, multi-select and confirmation work; ungrouped outlets already managed by the HQ are eligible in the picker | Retain the assignment |
| Outlet assignment | Add outlets directly to HQ | Built | Overview provides a bulk **Add outlets** flow. Eligible outlets can be searched, filtered and multi-selected; after adding them to the HQ, the user can either add them to an existing group or leave them ungrouped. The outlet type is updated to **Managed by HQ** | Replace the prototype catalogue and browser-only state with backend search and persistence |
...
| 20 August 2026 | Renamed the HQ action to **Add outlets**. Group pickers now include ungrouped outlets already managed by the HQ, and the HQ add flow offers an optional final step to place newly added outlets into an existing group or leave them ungrouped. |
