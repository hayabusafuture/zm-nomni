...
| HQ users | Grant/manage HQ access (PWF-1664) | Built | HQ user management already lives in the Create/Edit Buyer User flow (`Admin - Buyer User.html`, `Procure - User Detail.html`) — matches the ticket closely: Basic info first, Outlet + Permissions merged, then an HQ access section with an HQ-mode toggle, a Linked HQs multi-select, the three permissions (Manage user accounts, Grant HQ access, Revoke HQ access, disabled until an HQ is linked), an HQ Owner flag, and the "must be linked to at least one outlet or HQ" validation. A dead, unreachable "Users" tab-panel placeholder inside `Admin - HQ Settings.html` was removed and replaced with a working **Users** entry point in the HQ header (next to Edit details) that opens the Buyer User page with this HQ pre-linked and HQ mode pre-enabled | No page lists which users are already linked to a given HQ — the entry point only reaches the create/edit form. Admin has no user list at all (Procure does, via `Procure - Users.html`); roles beyond Regular/Owner and invitation flows are still undesigned |
