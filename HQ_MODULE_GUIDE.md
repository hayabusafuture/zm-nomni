...
- **Create item:** saves the new HQ catalogue record locally, shows an “Item created” confirmation, returns to the Items tab and refreshes the list. Defaults are optional (`S$0.00` / MOQ `0`). Its **Available to...** list follows the shared group/outlet selector: Select all, outlet groups first, then ungrouped outlets. Selecting a target reveals its available-UOM choices; global UOM toggles and per-target choices are retained.
- **Add from supplier catalogue:** starts with choosing the outlet groups and ungrouped outlets the items are for, then selecting catalogue items, then setting optional default price/MOQ values. There is no separate availability step: the initial target selection is applied to every added item. If a selected catalogue item already exists in the HQ, the flow updates its availability for the selected targets instead of skipping it, while preserving any UOM choices those targets already have.
